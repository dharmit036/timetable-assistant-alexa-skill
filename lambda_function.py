...
def supervisor():
    ...
    cursor.execute("select sup1,sup2,sup3,sup4,sup5 from supervisor where date=%s",(today))
...
def seniorsupervisor():
    ...
    now = datetime.datetime.now()
    today= now.strftime("%Y-%m-%d")
    cursor.execute("select sensup supervisor where date=%s",(today))
    rows = cursor.fetchall()
    for row in rows:
          a = rows
    speech_output =  str(a)     
...
def get_welcome_response():
    ...
    session_attributes = {}
    card_title = "Welcome"
    speech_output = "Welcome to Ganpat University"
...
def on_intent(intent_request, session):
    ...
    if intent_name == "supervisor":
        return supervisor()
    elif intent_name == "seniorsupervisor":
